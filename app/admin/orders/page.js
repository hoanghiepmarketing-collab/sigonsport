  ...
  return (
    <div style={{ padding: 28 }}>
      ...
      <div style={{ marginBottom: 16 }}>
        <input
          type="text" placeholder="Tìm mã đơn, tên, SĐT..."
      ...
      <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 3px rgba(0,0,0,.08)', border: '1px solid #e2e8f0', overflowX: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={8} style={{ padding: 24, textAlign: 'center', color: '#94a3b8' }}>Đang tải...</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={8} style={{ padding: 24, textAlign: 'center', color: '#94a3b8' }}>Không có đơn hàng</td></tr>
            ) : filtered.map(order => {
              ...
              return (
                <tr key={order.id} style={{ borderBottom: '1px solid #f8fafc' }}>
                  ...
                  <td style={{ padding: '12px 14px', color: '#475569' }}>
                    {Array.isArray(order.items) ? `${order.items.length} sản phẩm` : '—'}
                  </td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 16, width: '100%', maxWidth: 560, maxHeight: '90vh', overflowY: 'auto', padding: 28 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
              <h2 style={{ fontFamily: 'Montserrat, sans-serif', fontWeight: 800, fontSize: 18 }}>Chi tiết đơn {selected.order_number}</h2>
            ...
            <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: 12, marginBottom: 20, fontSize: 14 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, color: '#475569' }}>
                <span>Tạm tính</span><span>{formatPrice(selected.subtotal)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, color: '#475569' }}>
                <span>Vận chuyển</span><span>{selected.shipping === 0 ? 'Miễn phí' : formatPrice(selected.shipping)}</span>
              </div>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontWeight: 800, fontSize: 16, color: '#0f172a' }}>
                <span>Tổng cộng</span><span style={{ color: '#E5002B' }}>{formatPrice(selected.total)}</span>
              </div>
            </div>
                ...
                {STATUSES.filter(s => s !== 'all').map(s => {
                  ...
                  return (
                    <button key={s} disabled={updatingStatus || selected.status === s}
                      ...
                      style={{
                        padding: '6px 14px', borderRadius: 99, border: 'none', cursor: selected.status === s ? 'default' : 'pointer',
